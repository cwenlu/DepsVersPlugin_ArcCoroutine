 ...
 * 3.处理panel凸出部分是覆盖还是在main view下面
 * 4.处理fling 展开，折叠
 */
public class SimpleUseViewDragHelper2 extends ViewGroup {
    ...
    public SimpleUseViewDragHelper2(Context context) {
        this(context, null);
    }

    public SimpleUseViewDragHelper2(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SimpleUseViewDragHelper2(Context context, AttributeSet attrs, int defStyleAttr) {
...
//        requestLayout();
        //建议弄成xml属性，不然单独设置还需要重新measure，layout.设置得快可能不需要（巧合）
    }
...
//            必须 min=<top<=max
        ...
        public void onViewReleased(View releasedChild, float xvel, float yvel) {
            int target=0;
            if(yvel>0){
                target=getHeight()-mPeekHeight;
            } else{
                target=0;
            }
            mViewDragHelper.settleCapturedViewAt(releasedChild.getLeft(),target);
            invalidate();
        }
    }
